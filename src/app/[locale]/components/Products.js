    ...
    <section id="products" class="py-md-5 py-3">
      <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-md-5 mb-4">
          ...
          <Link href={"/store"} class="d-none d-md-block btn btn-outline-primary px-4">{t('read_more')}</Link>
        </div>
        ...
        <div class="text-center">
          <Link href={"/store"} class="d-md-none btn btn-outline-primary px-4 mt-4">{t('read_more')}</Link>
        </div>
      </div>
    </section>
  );
